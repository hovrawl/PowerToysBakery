...
using System.Text;
using Bitwarden.Client.Models;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;

namespace Bitwarden.Client;
...
public class BitwardenClient : IDisposable
{
    ...
    public string IssueBitWardenCommand(string cmd)
    {
        ...
        if (!string.IsNullOrWhiteSpace(errorResponse))
        {
            ...
            // These are the errors that we can safely ignore
            bool handled = errorResponse.Contains("you are already logged")
                           || (errorResponse.Contains("Could not find") && errorResponse.Contains("creating it instead"));
